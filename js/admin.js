...
Naqsh.Admin.renderUsers = async function() {
    ...
    h += '<div class="card" id="addUserSection" style="border:2px dashed var(--accent);background:#f0fdf9;margin-top:20px;display:none">' +
        ...
        '<button class="btn-ghost btn-sm" onclick="Naqsh.Admin._showAddUserSection()"><i class="fa-solid fa-xmark"></i></button></div>' +
        ...
        '<button class="btn btn-primary btn-full" style="padding:14px;font-size:15px;margin-bottom:10px" onclick="Naqsh.Admin._createUser()"><i class="fa-solid fa-user-plus"></i>إنشاء الحساب</button></div></div>';
...
Naqsh.Admin._showEditModal = function(uid) {
    ...
    if (!u) return;
    Naqsh.Utils.showModal(
        '<h3 style="font-size:18px;font-weight:700;margin-bottom:12px">تعديل بيانات المستخدم</h3>' +
...
Naqsh.Admin._saveEdit = async function(uid) {
    ...
    if (pass) {
        Naqsh.Utils.showToast('لتغيير كلمة المرور، استخدم Firebase Console', 'warning');
    }
    try {
